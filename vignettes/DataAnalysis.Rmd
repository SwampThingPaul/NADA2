...
```{r}
glm.null <- glm(GT_1~1,Recon,family=binomial(logit))
...
```{r}
bestglm(Recon,family = binomial(logit), IC = "AIC")
...
```{r}
Recon.frame = with(Recon, datadist(PctCorn, Dyplant, Pctl, Temp, GT_1))
options(datadist = "Recon.frame")
lrm4 <- lrm(GT_1 ~ PctCorn + Dyplant + Pctl + Temp, data = Recon)
lrm4
```
...
ent <- Mdat[, 13:14]   # the entero1A data
...
### Error ent.euclid and M6.euclid different dimensions
