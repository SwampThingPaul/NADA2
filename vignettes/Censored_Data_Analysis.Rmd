...
if you are using `setwd()`, for reproducibility it is recommend to use the code above where the path to the working directory is relative.
...
pkg <- c("bestglm","car","cenGAM","EnvStats","fitdistrplus","Kendall",
         "mgcv","multcomp","nlme","perm","rms","survminer",
         "vegan","NADA2","NADA","bestglm","remotes")
check.packages(pkg)

```


```{r,echo=F,warning=F,message=F,eval=F,results="hide",include = FALSE}
check.packages <- function(pkg){
  new.pkg <- pkg[!(pkg %in% installed.packages()[, "Package"])]
  if (length(new.pkg)) 
    install.packages(new.pkg, dependencies = TRUE)
...
pkg <- c("bestglm","car","cenGAM","EnvStats","fitdistrplus","Kendall",
         "mgcv","multcomp","nlme","perm","rms","survminer",
         "vegan","NADA","bestglm","remotes")
check.packages(pkg)
remotes::install_github("SwampThingPaul/NADA2")
# remotes::install_github("SwampThingPaul/NADA")
```
...
Currently the `NADA` package is no longer CRAN, but is still available on [GitHub](https://github.com/SwampThingPaul/NADA)
...
```{r load data example}
data(PbHeron); # From NADA package
...
### Exploring the data
In R, the summary command is used to briefly describe the characteristics of the data.
```{r sum}
...
```{r}
with(ShePyrene,NADA::censtats(Pyrene, PyreneCen))
```
...
The `PbHeron` data present lead concentrations in organs of herons after exposing them to lead.  There are four dosage groups (“Dosage” or “Group” columns), zero plus three amounts of lead.  The objective was to determine if feathers or another non-destructive part of the birds could be used as an environmental indicator, so it would not be necessary to sacrifice a bird in order to measure their lead concentrations.  A censored boxplot of the PbHeron liver lead data shows that the third and fourth groups have generally higher values.

```{r,fig.width=6.5,fig.height=4,fig.align='center'}
data(PbHeron)

with(PbHeron,cboxplot(Liver, LiverCen, Dosage,
...
```{r}
PbHeron$Below04 <- PbHeron$Liver
PbHeron$Below04[PbHeron$Liver<0.04] <- -1
PbHeron$Below04[PbHeron$LiverCen==TRUE] <- -1
...
```{r}
kruskal.test(Below04~Dosage,PbHeron)
```
...
```{r}
with(PbHeron,cen1way(Liver,LiverCen,Dosage))
```
...
```{r,fig.width=6.5,fig.height=4,fig.align='center'}
with(PbHeron,cen_ecdf(Liver,LiverCen,Dosage))
```

This is more easily seen by plotting the empirical cdfs in log units: 

```{r,fig.width=6.5,fig.height=4,fig.align='center'}
PbHeron$lnLiver=log(PbHeron$Liver)

with(PbHeron,cen_ecdf(lnLiver,LiverCen,Dosage,
...
```{r}
with(PbHeron,cenanova(Liver,LiverCen,Dosage))
```
...
```{r}
with(PbHeron,cenpermanova(Liver,LiverCen,Dosage))
```
...
```{r}
with(PbHeron,cenanova(Liver,LiverCen,Dosage,LOG=FALSE))
```
