...
cenQQ <- function(y.var, cen.var, dist = "lnorm", Yname = yname)  {
  ...
  norm.text <- paste("Shapiro-Francia W =", as.numeric(signif(var.choose$test.results$norm$statistic, 3) ))
  lnorm.text <- paste("Shapiro-Francia W =", as.numeric(signif(var.choose$test.results$lnorm$statistic, 3) ))
  gamma.text <- paste("Shapiro-Francia W =", as.numeric(signif(var.choose$test.results$gamma$statistic, 3) ))
