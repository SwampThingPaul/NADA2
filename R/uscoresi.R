...
#' Interval censored U-score (multiple values)
#'
#' @description Computes uscores within columns of interval-censored data (the "i") having one or more detection limits.
#' @param dat.frame A data frame. Default format is: paired = `TRUE`, the input format is ylo1 yhi1 ylo2 yhi2 ylo3 yhi3, etc., a pair of columns for each concentration parameter.  This is likely to be the most common arrangement, and so is the default.
#' @param paired An option to specify paired = `FALSE`, where the format would be ylo1 ylo2 ylo3 yhi1 yhi2 yhi3, low concentrations for each parameter followed by the high concentrations in the same order.
...
#' @param Cnames Cnames =`1` uses the "lo" column names to name the uscores columns.  Cnames = `2` uses the "hi" column names.
#' @details Input is a data.frame of paired low and high possible range of concentrations, in an interval-censored format. ylo = the lower end of the concentration interval is the first (left) column in the pair.  yhi is the upper end of the concentration interval, at the second (right) column in the pair. For a detected value, ylo=yhi.  For a ND,  ylo != yhi. The uscore = #obs known to be lower - #obs known to be higher. Ties, such as <1 vs <3 or 4 vs 4 or <3 vs 2, are 0s in the uscore computation.
#'
#' @export
#'
